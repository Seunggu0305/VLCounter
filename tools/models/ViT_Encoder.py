...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=384, patch_size=16, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.1, out_indices=[6,7,8,11], pretrained=None, get_embeddings=True, **kwargs):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.get_embeddings:
            ...
            if len(self.out_indices) == 1:
                ...
                features[-1] = (self.ln_post(features[-1].flatten(-2).permute(0,2,1)) @ self.proj).permute(0,2,1).reshape(B,-1, H, W) 
                features[-1] = features[-1] / features[-1].norm(dim=1, keepdim=True)
